...
military = {
	...
	LEADER_TRANSFER_PENALTY_ON_COUNTRY_PRESTIGE = 0.02, -- country prestige penalty on unassign leader (f.ex if leader has 100% prestige, the country loose 4% of its prestige)
	...
	NAVAL_COMBAT_SEEKING_CHANCE = 0.65, -- base chance of picking a target (increased by leader reconaissance)
	NAVAL_COMBAT_SEEKING_CHANCE_MIN = 0.1, -- low cap for chance of picking the target
	NAVAL_COMBAT_SELF_DEFENCE_CHANCE = 3.0, -- scale up the chance of choosing the target that is already shooting at us.
	NAVAL_COMBAT_SHIFT_BACK_ON_NEXT_TARGET = 0.6, -- max shift back from center line when got new target
	NAVAL_COMBAT_SHIFT_BACK_DURATION_SCALE = 100, -- shifting back from center line on new target - scaled down as long the combat takes
	...
	NAVAL_COMBAT_CHANGE_TARGET_CHANCE = 0.05, -- chance of changing the target to another one
	NAVAL_COMBAT_DAMAGE_ORG_MULT = 0.2, -- scale the damage to ORG for balancing gameplay
	NAVAL_COMBAT_DAMAGE_STR_MULT = 0.1, -- scale the damage to STR for balancing gameplay
	NAVAL_COMBAT_DAMAGE_MULT_NO_ORG = 5.0, -- damage multiplier (to STR) when opponent has no ORG.
	NAVAL_COMBAT_RETREAT_CHANCE = 0.3, -- base chance for retreating when STR or ORG is low. Increased by leader experiance and speed.
	NAVAL_COMBAT_RETREAT_STR_ORG_LEVEL = 0.25, -- the retreating is available when STR or ORG % drops below this value (it's 0% to 100% value)
	NAVAL_COMBAT_RETREAT_SPEED_MOD = 0.5, -- slow down when retreating so there is a chance to hit the runner
	NAVAL_COMBAT_RETREAT_MIN_DISTANCE = 0.60, -- how close (in average) all alive ships must be to the center line to be able to retreat.
	NAVAL_COMBAT_DAMAGED_TARGET_SELECTION = 2.5, -- multiply chance of seleting target that has low average of STR/ORG
	NAVAL_COMBAT_STACKING_TARGET_CHANGE = 0.10, -- increase chance to change/drop target when suffering stacking penalty
	NAVAL_COMBAT_STACKING_TARGET_SELECT = 0.4, -- modifier for how much the stacking penalty affects the target selection. (MIGHT WANT TO INCREASE THIS EVEN FURTHER??)
...
diplomacy = {
	...
	LARGE_POPULATION_INFLUENCE_PENALTY = -0.05,
